...
window.saveTower = async function() {
    ...
    localStorage.setItem('cachedTowers', JSON.stringify(towersArray));
...
function loadTowers() {
    towersArray = JSON.parse(localStorage.getItem('cachedTowers')) || [];
    allCustomers = JSON.parse(localStorage.getItem('cachedCustomers')) || [];
    
    if(towersArray.length === 0) {
        towersArray = [{ towerCode: "1234", towerName: "برج تجريبي (للعرض)", ownerName: "مدير تجريبي" }];
        localStorage.setItem('cachedTowers', JSON.stringify(towersArray));
    }
